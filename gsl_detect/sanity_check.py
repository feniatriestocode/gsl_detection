...
N_FACE   = len(FACE_SELECTED_INDICES)  # 44
POSE_END = 15 * 4                      # 60
FACE_END = POSE_END + N_FACE * 3       # 60 + 132 = 192
LH_END   = FACE_END + 63              # 255
RH_END   = LH_END + 63               # 318
...
def dataset_stats():
    ...
    for f in files:
        data = np.load(f)["sequence"]  # shape: (n_frames, n_features)
    ...
    # amount of short
    print(f"\nFiles with >50 and <70 frames: {len(short)}")
    # for f, n_frames in short:
    #     print(f"{n_frames} frames — {f.name}")
